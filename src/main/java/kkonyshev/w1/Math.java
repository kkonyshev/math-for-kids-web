...
public class Math extends WebPage {
	...
	public Math(PageParameters aParameters) {
        ...
        final NumberListView numberListView = new NumberListView("listview", form.getModel());
		...
		final Label countLabel = new Label("profileCount", new PropertyModel<Counter>(counterModel, "count"));
		AbstractAjaxTimerBehavior ajaxSelfUpdatingTimerBehavior = new AbstractAjaxTimerBehavior(Duration.seconds(1)) {
	...
	public class NumberListView extends ListView<NumberStat> {
		...
		protected void populateItem(ListItem<NumberStat> item) {
	        item.add(new Label("number", String.valueOf(item.getModelObject().getNumber())));
	        item.add(new Label("count", String.valueOf(item.getModelObject().getCount())));
	    }
    }
    ...
    private class ProfileNumberModel implements IModel<List<NumberStat>> {
		...
		public List<NumberStat> getObject() {
			List<NumberStat> numberCount = profileService.getNumberStat(store.getObject());
